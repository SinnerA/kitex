...
func newHTTP2Server(ctx context.Context, conn netpoll.Connection) (_ ServerTransport, err error) {
	...
	t := &http2Server{
		...
		fc:                &trInFlow{limit: defaultWindowSizeForDebug},
		state:             reachable,
		activeStreams:     make(map[uint32]*Stream),
		kp:                kp,
		kep:               kep,
		idle:              time.Now(),
		initialWindowSize: int32(defaultWindowSizeForDebug),
	...
	gofunc.RecoverGoFuncWithInfo(ctx, func() {
		t.loopy = newLoopyWriter(serverSide, t.framer, t.controlBuf, t.bdpEst, defaultWindowSizeForDebug, defaultWindowSizeForDebug)
