...
func newHTTP2Server(ctx context.Context, conn netpoll.Connection, config *ServerConfig) (_ ServerTransport, err error) {
	...
	if config.InitialWindowSize > 0 && config.InitialWindowSize != defaultWindowSize {
	...
	if config.InitialConnWindowSize > 0 && config.InitialConnWindowSize != defaultWindowSize {
	...
	// kp.MaxConnectionAge += getJitter(kp.MaxConnectionAge) // FIXME: impl the getJitter
	...
	gofunc.RecoverGoFuncWithInfo(ctx, func() {
		t.loopy = newLoopyWriter(serverSide, t.framer, t.controlBuf, t.bdpEst, 65535)
